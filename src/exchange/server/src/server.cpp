...
void Server::processRequest(InboundState& state) {
    ...
    while (buf.length() >= INBOUND_BSIZE) {
        ...
        if (memcmp(data, "EXCHANGE\n", header_len) != 0) {
            ...
            while (!error_channel_.push(pe) && !stop_.load(std::memory_order_relaxed)) {}
        ...
        while (!in_ring_.push(msg) && !stop_.load(std::memory_order_relaxed)) {}
    }
}
...
void Server::routeOutboundMessages() {
    ...
    std::optional<OutboundMessage> cur = out_ring_.pop();
    int cur_fd = cur ? resolve(*cur) : -1;
    if (cur_fd > 0) prefetch_read(&outbound_map_[cur_fd]);

    while (cur) {
        std::optional<OutboundMessage> nxt = out_ring_.pop();
        int nxt_fd = -1;
        if (nxt) {
            nxt_fd = resolve(*nxt);
        ...
        if (cur_fd > 0 && !condemned_inbound_[cur_fd].load(std::memory_order_relaxed)) {
            ...
            if (ostate.has_value()) {
                if (ostate->push_outbound(*cur)) {
                    writable_fds_.insert(cur_fd);
                } else {
                    // Staging ring is full even though we drain it every
                    // serveResponses() pass: this connection can't keep up.
                    // Condemn it (same as a failed send) instead of dropping
                    // the message, so the client's own reconnect logic resets
                    // its pipeline-depth accounting instead of stalling forever.
                    condemnConnection(cur_fd, false);
                }
            }
        }
    ...
    while (auto pe = error_channel_.pop()) {
        const int fd = pe->fd;
        if (condemned_inbound_[fd].load(std::memory_order_relaxed)) continue;
        ...
        if (ostate->push_outbound(OutboundMessage{
