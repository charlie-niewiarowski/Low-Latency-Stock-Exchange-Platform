...
public:
    ...
    // Returns false if the staging ring is full (caller should treat this
    // connection as a slow consumer and condemn it — see server.cpp's
    // routeOutboundMessages — rather than silently dropping the message,
    // which would leave the client's own pipeline-depth accounting stuck
    // waiting for a response that will never arrive).
    [[nodiscard]] bool push_outbound(OutboundMessage msg) {
        if (!staging_.push(msg)) return false;
        ++pending_count_;
        return true;
    }
